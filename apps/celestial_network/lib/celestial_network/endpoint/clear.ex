defmodule CelestialNetwork.Endpoint.Clear do
  @moduledoc false
  ...
  defp init(socket, transport, opts) do
    ...
    handshake_loop(handler, pubsub_server, opts, [conn])
  end

  defp handshake_loop(handler, pubsub_server, opts, [conn | state]) do
    ...
    receive do
      {^ok, ^socket, data} ->
        handle_handshake(handler, data, pubsub_server, opts, [conn | state])
      ...
      _ ->
        handshake_loop(handler, pubsub_server, opts, [conn | state])
    end
  rescue
    e ->
  ...
  defp handle_handshake(handler, data, pubsub_server, opts, [conn | _]) do
