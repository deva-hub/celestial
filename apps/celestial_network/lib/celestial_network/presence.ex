defmodule CelestialNetwork.Presence do
  @moduledoc """
  ...
  provided by `CelestialNetwork.Presence`, such as tracking processes
  ...
  which uses `CelestialNetwork.Presence` and provide the `:otp_app` which
  ...
  """
  @callback track(socket :: CelestialNetwork.Socket.t(), key :: String.t(), meta :: map()) ::
              {:ok, ref :: binary()}
              | {:error, reason :: term()}
  ...
  """
  @callback untrack(socket :: CelestialNetwork.Socket.t(), key :: String.t()) :: :ok
  ...
  """
  @callback update(
              socket :: CelestialNetwork.Socket.t(),
              key :: String.t(),
              meta :: map() | (map() -> map())
            ) ::
              {:ok, ref :: binary()}
  ...
  `:phx_ref` value.
  """
  @callback list(CelestialNetwork.Socket.t() | topic) :: presences
  ...
  """
  @callback get_by_key(CelestialNetwork.Socket.t() | topic, key :: String.t()) :: presences
  ...
  defmacro __using__(opts) do
    quote location: :keep, bind_quoted: [opts: opts] do
      @behaviour CelestialNetwork.Presence
      @opts opts
      @task_supervisor Module.concat(__MODULE__, "TaskSupervisor")

      _ = opts[:otp_app] || raise "use CelestialNetwork.Presence expects :otp_app to be given"
      ...
      def child_spec(opts) do
        ...
        %{
          id: __MODULE__,
          start: {CelestialNetwork.Presence, :start_link, [__MODULE__, @task_supervisor, opts]},
          type: :supervisor
        }
      end
      ...
      def track(%CelestialNetwork.Socket{} = socket, key, meta) do
      ...
      def untrack(%CelestialNetwork.Socket{} = socket, key) do
      ...
      def update(%CelestialNetwork.Socket{} = socket, key, meta) do
      ...
      def list(%CelestialNetwork.Socket{topic: topic}), do: list(topic)
      def list(topic), do: CelestialNetwork.Presence.list(__MODULE__, topic)

      def get_by_key(%CelestialNetwork.Socket{topic: topic}, key), do: get_by_key(topic, key)
      def get_by_key(topic, key), do: CelestialNetwork.Presence.get_by_key(__MODULE__, topic, key)
  ...
  defmodule Tracker do
    ...
    def start_link({module, task_supervisor, opts}) do
      pubsub_server =
        opts[:pubsub_server] ||
          raise "use CelestialNetwork.Presence expects :pubsub_server to be given"
      ...
      Task.Supervisor.start_child(task_supervisor, fn ->
        for {topic, {joins, leaves}} <- diff do
          Phoenix.PubSub.local_broadcast(pubsub_server, topic, %Message{
            event: "presence_diff",
            payload: %{
              joins: module.fetch(topic, CelestialNetwork.Presence.group(joins)),
              leaves: module.fetch(topic, CelestialNetwork.Presence.group(leaves))
            }
          })
        end
      end)
