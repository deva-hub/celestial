defmodule Nostalex.Presence do
  @moduledoc """
  ...
  provided by `Nostalex.Presence`, such as tracking processes
  ...
  which uses `Nostalex.Presence` and provide the `:otp_app` which
  ...
  defmacro __using__(opts) do
    quote location: :keep, bind_quoted: [opts: opts] do
      @behaviour Nostalex.Presence
      @opts opts
      @task_supervisor Module.concat(__MODULE__, "TaskSupervisor")

      _ = opts[:otp_app] || raise "use Nostalex.Presence expects :otp_app to be given"
      ...
      def child_spec(opts) do
        ...
        %{
          id: __MODULE__,
          start: {Nostalex.Presence, :start_link, [__MODULE__, @task_supervisor, opts]},
          type: :supervisor
        }
      end
